...
function OpenButcherSellMenu()
    ...
    MenuData.Open('default', 'butcher_menu', 'main', {
        ...
        local selectedItem = tostring(data.current.value or ""):match("^%s*(.-)%s*$")
        print("[DEBUG] RAW VALUE:\n" .. tostring(data.current.value))
        print("[DEBUG] SELECTED ITEM:\n" .. selectedItem)
        print("[DEBUG] DATA:\n" .. tostring(data))
        print("[DEBUG] Sending item to server for sale:", selectedItem)

        if selectedItem ~= "" and selectedItem ~= "none" then
            TriggerServerEvent("vorp_hunting:sellButcherItem", selectedItem)
            menu.close()
        end
    ...
    while true do
        ...
        for i, v in ipairs(Config.Butchers) do
            ...
            if distance <= v.radius then
                ...
                if Citizen.InvokeNative(0xC92AC953F0A982AE, openButcher) then
                    print("[DEBUG] G pressed inside radius")
                    if not pressed then
                        pressed = true
                        OpenButcherSellMenu()
                    end
