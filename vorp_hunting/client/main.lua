...
local function trim(str)
    return (str and type(str) == "string") and str:match("^%s*(.-)%s*$") or ""
end

function OpenButcherSellMenu()
    local playerItems = exports.vorp_inventory:getInventoryItems()
    local menuItems = {}
    ...
    for _, item in pairs(playerItems) do
        ...
        local pricePer = Config.ButchersItemPrices[itemName]
        if pricePer and itemCount > 0 then
            table.insert(menuItems, {
                label = ("%s (%sx) - $%s"):format(itemLabel, itemCount, pricePer),
                value = itemName,
                desc = "Press ENTER to sell all",
                type = "button",
                count = itemCount
            })
        end
    ...
    if #menuItems == 0 then
        table.insert(menuItems, {
            label = "No sellable animal items",
            value = "none",
            type = "button"
        })
    end

    MenuData.Open('default', 'butcher_menu', 'main', {
        title = "Butcher Shop",
        elements = menuItems
    }, function(data, menu)
        local selectedItem = trim(tostring(data.current.value))
        local itemCount = tonumber(data.current.count) or 1

        print("[DEBUG] Selected:", selectedItem, "Count:", itemCount)

        if selectedItem ~= "" and selectedItem ~= "none" and itemCount > 0 then
            TriggerServerEvent("vorp_hunting:sellInventoryItem", selectedItem, itemCount)
        else
            print("[ERROR] Invalid selection or count.")
        end

        menu.close()
    end, function(data, menu)
        menu.close()
        TriggerEvent("vorp_hunting:unlock") -- Ensure unlock on cancel
    end)
end
...
local function SellAnimal()
    ...
    if holding and not alreadysoldanimal then
    ...
    -- FIXED: Inventory fallback (using getInventoryItems)
    if not alreadysoldanimal then
        ...
        for _, item in pairs(playerItems) do
            ...
            if Config.ButchersItemPrices[itemName] and itemCount > 0 then
                TriggerServerEvent("vorp_hunting:sellInventoryItem", itemName, itemCount)
                soldSomething = true
                alreadysoldanimal = true
                break -- remove this line if you want to sell ALL matching items
            end
        end
        ...
        for i, v in ipairs(Config.Butchers) do
            ...
            if distance <= v.radius then
                ...
                if Citizen.InvokeNative(0xC92AC953F0A982AE, openButcher) then
                    if not pressed then
                        pressed = true
                        print("[DEBUG] G pressed - opening menu")
                        OpenButcherSellMenu()
                        -- fallback unlock in case server doesn’t respond
                        SetTimeout(5000, function()
                            if pressed then
                                print("[DEBUG] Forced unlock fallback")
                                TriggerEvent("vorp_hunting:unlock")
                            end
                        end)
                    end
                end
